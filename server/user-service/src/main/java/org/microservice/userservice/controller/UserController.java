...
public class UserController {
    ...
    @GetMapping("/{authId}")
    public ResponseEntity<ApiResponse<User>> getUser(@PathVariable("authId") Long authId){
        try {
            return ResponseEntity.ok(ApiResponse.success(userService.getUserByAuthId(authId)));
        }catch (Exception e){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(ApiResponse.error(e.getMessage()));
        }
