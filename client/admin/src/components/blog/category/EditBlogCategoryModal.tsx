...
import type { UpdateBlogCategoryFormModalProps } from "../../../types/blog/category/category.type";
import { X } from "lucide-react";
import { getBlogCategoryById, updateBlogCategory } from "../../../service/api/Blog/BlogCategory";
...
const EditBlogCategoryModal = ({
  ...
  onSuccess,
  blogCategoryId,
}: UpdateBlogCategoryFormModalProps) => {
  ...
  useEffect(() => {
    if (!isOpen || !blogCategoryId) return;
    setLoading(true);
  ...
  }, [isOpen, blogCategoryId]);
  const handleSubmit = async ()=>{
    setSaving(true);
    try {
      await updateBlogCategory(blogCategoryId!, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-lg ring-1 ring-[#E9EEF5]">
        <div className="flex items-center justify-between border-b border-[#EEF2F7] px-5 py-4">
          <h2 className="text-base font-semibold text-[#1E293B]">Edit Blog Category</h2>
          <button
            onClick={onClose}
