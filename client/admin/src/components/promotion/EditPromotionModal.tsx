...
import type { UpdatePromotionModalProps } from "../../types/promotion/promotion";
...
const EditPromotionModal = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await updatePromotion(promotionId, cleanedData);
      showAlert({
        title: response?.data?.message || "Promotion update successfully",
        type: "success",
        autoClose: 3000,
      });
      setFormData({
      ...
      showAlert({
        title: err?.response?.data?.message || "Promotion update failed. Please try again later.",
        type: "error",
        autoClose: 4000,
      });
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-lg ring-1 ring-[#E9EEF5]">
        <div className="flex items-center justify-between border-b border-[#EEF2F7] px-5 py-4">
          <h2 className="text-base font-semibold text-[#1E293B]">Edit Promotion</h2>
          <button
            onClick={onClose}
          ...
          <div className="space-y-4 px-5 py-4">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="space-y-2">
                ...
                <div className="h-9 w-full rounded-xl bg-[#F1F5F9] animate-pulse" />
              </div>
            ))}
          </div>
        ) : (
