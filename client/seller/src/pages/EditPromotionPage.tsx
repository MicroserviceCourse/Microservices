...
const EditPromotionPage = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = await updatePromotion(Number(id),payload);

            showAlert({
                title: response?.data?.message || "Promotion updated successfully!",
                type: "success",
                autoClose: 3000,
            });
            setForm({
    ...
    useEffect(() => {
        ...
        const fetchPromotion = async () => {
            try {
                const response = await getPromotionById(Number(id));
        ...
        <div className="p-8 bg-gray-50 min-h-screen space-y-8">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                    <button onClick={()=>navigate("/Dashboard/promotion")} className="px-4 py-2 rounded-lg  bg-slate-200 hover:bg-slate-300 font-medium text-[#334155">
                        Cancel
                    </button>
                    <button
                        disabled={isSaving}
